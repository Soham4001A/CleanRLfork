    ...
    def thunk():
        ...
        if capture_video and idx == 0:
            if env_id == "f16_intercept":
                ...
                env = F16InterceptEnv(render_mode="rgb_array")
            else:
...
class Agent(nn.Module):
    """
    Policy/Value network that views the 10‑step observation window
    (10 × 26 = 260 dims) as a sequence of 10 tokens, each 26‑d.
    A small Transformer encoder processes the sequence; the mean
    pooled embedding feeds separate linear heads for actor & critic.
    """

    SEQ_LEN  = 10      # OBS_WINDOW in env
    TOK_DIM  = 26
    D_MODEL  = 128
    N_HEADS  = 8
    N_LAYERS = 4
    ...
    def __init__(self, envs):
        ...
        self.token_embed = nn.Linear(self.TOK_DIM, self.D_MODEL)
        self.pos_embed   = nn.Parameter(torch.randn(1, self.SEQ_LEN, self.D_MODEL))

        # --- Transformer encoder ----------------------------------
        enc_layer = nn.TransformerEncoderLayer(
            d_model=self.D_MODEL, nhead=self.N_HEADS,
            dim_feedforward=self.D_MODEL * 4, batch_first=True, activation="gelu"
        )
        self.transformer = nn.TransformerEncoder(enc_layer, num_layers=self.N_LAYERS)

        # --- Actor / Critic heads ---------------------------------
        self.critic_head = layer_init(nn.Linear(self.D_MODEL, 1), std=1.0)
        self.actor_mean  = layer_init(
            nn.Linear(self.D_MODEL, act_space_shape_prod), std=0.01
        )
    ...
    def _encode(self, flat_obs: torch.Tensor) -> torch.Tensor:
        """
        Convert (batch, 260) -> (batch, 10, 26) -> embed -> Transformer
        returns pooled (batch, D_MODEL) representation.
        """
        B = flat_obs.shape[0]
        seq = flat_obs.view(B, self.SEQ_LEN, self.TOK_DIM)
        x   = self.token_embed(seq) + self.pos_embed  # (B,10,D_MODEL)
        x   = self.transformer(x)                     # (B,10,D_MODEL)
        return x.mean(dim=1)                          # mean‑pool
    ...
    def get_action_and_value(self, x, action=None):
        ...
        # sanitise NaNs/Infs that may have crept in
        action_mean   = torch.nan_to_num(action_mean, nan=0.0, posinf=1e6, neginf=-1e6)
        action_logstd = torch.nan_to_num(action_logstd, nan=0.0, posinf=10.0, neginf=-10.0)

        # Clamp log‑std to a reasonable range to avoid zero/inf std
        action_logstd = torch.clamp(action_logstd, -6.0, 2.0)
        action_std = torch.exp(action_logstd)

        probs = Normal(action_mean, action_std)
        if action is None:
            action = probs.rsample()  # re‑param sample for better gradients
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps): # This is the rollout loop for standard PPO
            ...
            with torch.no_grad():
                obs_clean = torch.nan_to_num(next_obs, nan=0.0, posinf=1e6, neginf=-1e6)
                action, logprob, _, value = agent.get_action_and_value(obs_clean)
        ...
        # ── console stats ───────────────────────────────────────────
        if args.num_envs == 1 and "final_info" in infos and infos["final_info"][0]:
            ep_info = infos["final_info"][0]["episode"]
            print(f"[it {iteration:04d}]  ep_ret={ep_info['r']:.1f}  "
                  f"len={ep_info['l']}  last_rew={reward:.2f}")
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                mb_obs_clean = torch.nan_to_num(b_obs[mb_inds], nan=0.0, posinf=1e6, neginf=-1e6)
                _, newlogprob, entropy, newvalue = agent.get_action_and_value(mb_obs_clean, b_actions[mb_inds])
